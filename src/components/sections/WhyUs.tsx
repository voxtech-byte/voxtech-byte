...
export default function WhyUs() {
  return (
    <section id="why-us" className="py-24 md:py-32 px-0 md:px-24 bg-slate-100 relative overflow-hidden bg-dot-grid">
      {/* Decorative Lines Parallax Effect & Vertical Boundaries */}
      <div className="absolute top-0 left-0 w-full h-full opacity-30 pointer-events-none">
        <div className="absolute top-1/4 right-0 w-[40rem] h-[1px] bg-slate-900/5 rotate-[25deg]"></div>
        <div className="absolute top-2/4 right-0 w-[40rem] h-[1px] bg-slate-900/5 rotate-[25deg]"></div>
        {/* Thin vertical lines on desktop */}
        <div className="hidden md:block absolute top-0 left-12 w-[1px] h-full bg-slate-900/5"></div>
        <div className="hidden md:block absolute top-0 right-12 w-[1px] h-full bg-slate-900/5"></div>
      </div>
      ...
      <div className="flex flex-nowrap md:flex-col overflow-x-auto snap-x-mandatory hide-scrollbar pb-8 px-6 md:px-0 md:max-w-7xl md:mx-auto md:space-y-48 gap-8 md:gap-0 relative z-10">
        {config.whyUs.points.map((pt, idx) => (
          <div key={idx} className={`snap-center flex-shrink-0 w-[85vw] md:w-auto flex flex-col md:flex-row items-center gap-8 md:gap-16 ${idx % 2 !== 0 ? 'md:flex-row-reverse' : ''} animate-on-scroll relative`}>
            ...
            <div className="hidden md:block absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[12rem] lg:text-[20rem] font-headline font-extrabold text-slate-900/5 leading-none select-none pointer-events-none z-0 float-number">
              {pt.id}
            </div>

            <div className="max-w-xl relative z-10 bg-white/50 backdrop-blur-sm p-8 md:p-10 rounded-2xl md:bg-transparent md:border-none md:backdrop-blur-none md:shadow-none">
              <h3 className="font-headline font-bold text-2xl md:text-3xl mb-4 md:mb-6 text-blue-950 border-l-4 border-blue-600 pl-4">{pt.title}</h3>
              <p className="text-slate-600 leading-relaxed md:text-lg font-body">
                {pt.description}
              </p>
            </div>
            ...
            {/* Image with Tilt effect on desktop */}
            <div className="hidden md:block">
              <TiltCard
                className={`w-80 h-96 rounded-xl overflow-hidden editorial-shadow relative z-30 ${idx % 2 === 0 ? 'rotate-3' : '-rotate-6'}`}
                rotation={idx % 2 === 0 ? 'rotate(3deg)' : 'rotate(-6deg)'}
              >
            ...
            {/* Mobile image — no tilt, simple display */}
            <div className="md:hidden w-full h-72 rounded-xl overflow-hidden editorial-shadow relative z-30">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img
                src={pt.image}
                alt={pt.title}
      ...
      {/* Transisi Wave ke Section Portfolio (bg-slate-200) */}
      <div className="wave-divider text-slate-200 relative z-20">
