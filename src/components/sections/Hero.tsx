...
import HeroEffects from "@/components/utils/HeroEffects";

export default function Hero() {
  return (
    <section
      data-hero-parallax
      className="relative min-h-screen flex flex-col justify-center px-6 md:px-12 lg:px-24 pt-32 pb-12 overflow-hidden bg-dot-grid"
    >
      <HeroEffects />

      {/* Morphing Blob 1 */}
      <div
        className="morph-blob-1 absolute top-[-10%] right-[-5%] w-[500px] h-[500px] bg-blue-600/20 pointer-events-none"
        data-parallax-layer="0.3"
      ></div>

      {/* Morphing Blob 2 */}
      <div
        className="morph-blob-2 absolute bottom-[-15%] left-[-10%] w-[400px] h-[400px] bg-indigo-400/15 pointer-events-none"
        data-parallax-layer="0.2"
      ></div>

      {/* Large Decorative Typography — Layer 1 (slow) */}
      <div
        className="absolute left-[-5%] md:left-12 top-1/4 select-none pointer-events-none opacity-[0.03] text-[40rem] font-headline font-black leading-none text-primary"
      ...
      <div
        className="absolute top-32 left-12 w-6 h-6 border-2 border-primary/20 rounded-full pointer-events-none animate-pulse hidden md:block"
        data-parallax-layer="1.0"
      ></div>
      <div
        className="absolute bottom-48 left-1/3 w-2 h-2 bg-primary/20 rounded-full pointer-events-none hidden lg:block"
        data-parallax-layer="1.5"
      ></div>

      {/* Main Content — Layer 3 (subtle follow) */}
      <div
        className="grid grid-cols-12 gap-6 items-center max-w-screen-2xl mx-auto w-full relative z-10"
        data-parallax-layer="0.15"
      >
      ...
      {/* Transisi Wave ke Section WhyUs (bg-slate-100) */}
      <div className="wave-divider text-slate-100">
